...
class PostulantJobOfferController extends Controller
{
    ...
    public function rejectAllPending($jobOfferId)
    {
        JobApplication::where('job_offer_id', $jobOfferId)

                ->where('status', 1)  // 1 es el estado de "Pendiente"
                ->update(['status' => 3]);  // 3 es el estado de "Rechazado"

        return redirect()->back()->with('success', 'Todos los postulantes pendientes han sido rechazados.');
    }
